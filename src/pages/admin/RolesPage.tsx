...
export default function RolesPage(){
  ...
  useEffect(()=>{return onSnapshot(collection(db,'roles'),async snap=>{const d:Record<string,any>={};snap.docs.forEach(doc=>d[doc.id]={id:doc.id,...doc.data()});
    for(const{id}of SR){if(!d[id])await setDoc(doc(db,'roles',id),{name:ROLE_LABELS[id],description:SR.find(r=>r.id===id)?.desc||'',permissions:DEFAULT_PERMISSIONS[id]||{},isSystem:true,createdAt:serverTimestamp()});}
  ...
  return(<div className="space-y-6 max-w-4xl">
    <div><h1 className="text-2xl font-black text-navy dark:text-white uppercase tracking-tight">Roles & Permissions</h1>
      <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">{isSA?'Click Edit on any role to change permissions.':'View what each role can access.'}</p></div>
      ...
      return(<div key={id} className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm overflow-hidden">
        <div className="flex items-center gap-3 px-5 py-4">
          ...
          <div className="flex items-center gap-2 shrink-0">
            {isEdit?<>
              <button onClick={()=>setEditId(null)} className="flex items-center gap-1 px-3 py-1.5 text-slate-500 hover:text-navy dark:text-slate-400 dark:hover:text-white rounded-xl text-xs font-bold border border-slate-200 dark:border-slate-600"><X className="w-3 h-3"/>Cancel</button>
        ...
        {isExp&&<div className="px-5 pb-5 border-t border-slate-100 dark:border-slate-700 pt-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {PL.map(({key,label,desc:pd})=>{const on=perms[key];return(
              <div key={key} onClick={()=>isEdit&&!locked&&setEditPerms(p=>({...p,[key]:!p[key]}))}
                className={`flex items-center gap-3 p-3 rounded-xl transition-all ${isEdit&&!locked?'cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-700':''} ${on?'bg-green-50 dark:bg-green-900/20':''}`}>
                <div className={`w-5 h-5 rounded-lg flex items-center justify-center shrink-0 ${on?'bg-green-500':'bg-slate-200 dark:bg-slate-600'}`}>
