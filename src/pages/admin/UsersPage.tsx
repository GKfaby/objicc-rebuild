...
export default function UsersPage(){
  ...
  const{showToast}=useToast();const{profile:me,permissions}=useUser();
  const isPrivileged=me?.role==='super_admin'||me?.role==='admin';
  const canManageTarget=(u:UserProfile)=>{
    if(isPrivileged)return true;
    const protectedTarget=['super_admin','admin'].includes(u.role);
    return!!permissions.manageUsers&&(!protectedTarget||!!permissions.manageProtectedUsers);
  };
  // Roles selectable in the dropdown for a given user -- hides Super
  // Admin/Admin as options entirely unless the acting user could actually
  // assign them, so there's no way to promote someone into a protected
  // role from here without the right permission.
  const selectableRoles=(u:UserProfile)=>{
    const allowed=isPrivileged||!!permissions.manageProtectedUsers;
    return allowed?SYSTEM_ROLES:SYSTEM_ROLES.filter(r=>!['super_admin','admin'].includes(r)||r===u.role);
  };
  ...
  const changeRole=async(u:UserProfile,r:Role)=>{
    const becomesProtected=['super_admin','admin'].includes(r);
    if(!isPrivileged&&(!canManageTarget(u)||(becomesProtected&&!permissions.manageProtectedUsers))){showToast('Only Super Admin/Admin can assign or change this role.','error');return;}
    if(u.uid===me?.uid&&r!=='super_admin'&&!window.confirm('Change your own role?'))return;
    await updateDoc(doc(db,'users',u.uid),{role:r,updatedAt:serverTimestamp()});await notify(u.uid,'Role Updated',`Your role has been changed to ${ROLE_LABELS[r]}.`,'info');showToast(`${u.displayName} -> ${ROLE_LABELS[r]}`,'success');};
  ...
  const performDelete=async(u:UserProfile)=>{
    ...
    if(!canManageTarget(u)){showToast('Only Super Admin/Admin can delete this account.','error');setConfirmDelete(null);return;}
  ...
  const performBanToggle=async(u:UserProfile)=>{
    ...
    if(!canManageTarget(u)){showToast('Only Super Admin/Admin can restrict this account.','error');setConfirmBan(null);return;}
    const next=!u.banned;
    ...
    {tab==='rejected'&&(rejected.length===0?<p className="text-slate-400 text-sm py-8 text-center">No rejected applications.</p>:
      <div className="space-y-2 max-h-[560px] overflow-y-auto pr-1">{rejected.map(u=><div key={u.uid} className="bg-slate-100 dark:bg-slate-700/40 border border-slate-200 dark:border-slate-600 rounded-2xl p-4 flex items-center gap-4 flex-wrap">
        ...
        <div className="flex gap-2 shrink-0">
          ...
          <button onClick={()=>approveRole(u)} className="flex items-center gap-1 px-3 py-2 bg-green-500 hover:bg-green-600 text-white rounded-xl text-xs font-black uppercase tracking-widest"><Check className="w-3 h-3"/>Approve</button>
          <button onClick={()=>setConfirmDelete(u)} disabled={!canManageTarget(u)} title={canManageTarget(u)?'':'Only Super Admin/Admin (or a role granted Manage Protected Users) can delete this account'} className="flex items-center gap-1 px-3 py-2 bg-red-100 hover:bg-red-200 dark:bg-red-900/30 dark:hover:bg-red-900/50 text-red-700 dark:text-red-300 rounded-xl text-xs font-black uppercase tracking-widest disabled:opacity-40 disabled:cursor-not-allowed"><Trash2 className="w-3 h-3"/>Delete</button>
        </div>
      </div>)}</div>)}
    ...
    {tab==='current'&&<div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm overflow-hidden">
      ...
      <table className="w-full text-sm">
        ...
        :others.map(u=><tr key={u.uid} className="border-b border-slate-50 dark:border-slate-700/50 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-700/30">
          ...
          <td className="px-4 py-3"><select value={u.role} onChange={e=>changeRole(u,e.target.value as Role)} disabled={!canManageTarget(u)} title={canManageTarget(u)?'':'Only Super Admin/Admin can change this account\'s role'} className={`px-2 py-1.5 rounded-lg text-xs font-black outline-none border-none cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed ${RC[u.role]||'bg-slate-100 text-slate-700'}`}>{selectableRoles(u).map(r=><option key={r} value={r}>{ROLE_LABELS[r]}</option>)}</select></td>
          ...
          <td className="px-4 py-3 text-right"><div className="flex items-center justify-end gap-1">
            ...
            <button onClick={()=>setConfirmBan(u)} disabled={!canManageTarget(u)} className={`p-1.5 rounded-lg disabled:opacity-30 disabled:cursor-not-allowed ${u.banned?'text-green-500 hover:bg-green-50 dark:hover:bg-green-900/20':'text-amber-500 hover:bg-amber-50 dark:hover:bg-amber-900/20'}`} title={!canManageTarget(u)?'Only Super Admin/Admin can restrict this account':u.banned?'Restore access':'Restrict user'}>{u.banned?<ShieldOff className="w-4 h-4"/>:<Ban className="w-4 h-4"/>}</button>
            <button onClick={()=>setConfirmDelete(u)} disabled={!canManageTarget(u)} className="p-1.5 text-red-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg disabled:opacity-30 disabled:cursor-not-allowed" title={!canManageTarget(u)?'Only Super Admin/Admin can delete this account':'Delete'}><Trash2 className="w-4 h-4"/></button>
          </div></td>
        </tr>)}</tbody>
      </table>
      </div>
    ...
    {selected&&(<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-navy/50 backdrop-blur-sm">
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md">
        <div className="flex items-center gap-3 px-6 py-4 border-b border-slate-100 dark:border-slate-700">
          ...
          <div className="flex-1 min-w-0"><p className="font-black text-navy dark:text-white truncate">{selected.displayName}</p><span className={`px-2 py-0.5 rounded-full text-xs font-black ${RC[selected.role]||'bg-slate-100 text-slate-700'}`}>{ROLE_LABELS[selected.role]}</span></div>
        ...
        {selected.status==='rejected'&&<div className="px-6 pb-4 flex gap-2">
          <button onClick={()=>approveRole(selected)} className="flex-1 flex items-center justify-center gap-2 py-3 bg-green-500 hover:bg-green-600 text-white font-black rounded-xl text-xs uppercase tracking-widest"><Check className="w-4 h-4"/>Approve</button>
          <button onClick={()=>setConfirmDelete(selected)} disabled={!canManageTarget(selected)} title={canManageTarget(selected)?'':'Only Super Admin/Admin can delete this account'} className="flex-1 flex items-center justify-center gap-2 py-3 bg-red-100 hover:bg-red-200 dark:bg-red-900/30 dark:hover:bg-red-800/40 text-red-700 dark:text-red-300 font-black rounded-xl text-xs uppercase tracking-widest disabled:opacity-40 disabled:cursor-not-allowed"><Trash2 className="w-4 h-4"/>Delete</button>
        </div>}
        <div className="px-6 pb-5 border-t border-slate-100 dark:border-slate-700 pt-4 space-y-2">
          ...
          <select value={selected.role} onChange={e=>{changeRole(selected,e.target.value as Role);setSelected({...selected,role:e.target.value as Role});}} disabled={!canManageTarget(selected)} title={canManageTarget(selected)?'':'Only Super Admin/Admin can change this account\'s role'} className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-700 rounded-xl text-navy dark:text-white font-bold text-sm outline-none border border-slate-200 dark:border-slate-600 disabled:opacity-40 disabled:cursor-not-allowed">
            {selectableRoles(selected).map(r=><option key={r} value={r}>{ROLE_LABELS[r]}</option>)}
          </select>
          <button onClick={()=>setConfirmBan(selected)} disabled={!canManageTarget(selected)} title={canManageTarget(selected)?'':'Only Super Admin/Admin can restrict this account'} className={`w-full py-2.5 font-bold rounded-xl text-xs uppercase tracking-widest transition-colors flex items-center justify-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed ${selected.banned?'text-green-600 dark:text-green-400 hover:bg-green-50 dark:hover:bg-green-900/20':'text-amber-600 dark:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-900/20'}`}>{selected.banned?<><ShieldOff className="w-4 h-4"/>Restore Access</>:<><Ban className="w-4 h-4"/>Restrict User</>}</button>
          <button onClick={()=>setConfirmDelete(selected)} disabled={!canManageTarget(selected)} title={canManageTarget(selected)?'':'Only Super Admin/Admin can delete this account'} className="w-full py-2.5 text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 font-bold rounded-xl text-xs uppercase tracking-widest transition-colors disabled:opacity-40 disabled:cursor-not-allowed">Delete User</button>
        </div>
      </div>
    </div>)}
