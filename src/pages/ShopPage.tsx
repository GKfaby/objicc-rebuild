...
export default function ShopPage(){
  ...
  const addToCart=(item:Merchandise)=>{setCart(prev=>{const ex=prev.find(c=>c.id===item.id);if(ex)return prev.map(c=>c.id===item.id?{...c,quantity:c.quantity+1}:c);return[...prev,{id:item.id,name:item.name,price:item.price,quantity:1,image:item.image}];});showToast(`${item.name} added to cart`,'success');setCartOpen(true);};
  ...
  const totalPrice=()=>{const sum=cart.reduce((s,c)=>{const p=parseFloat(c.price.replace(/[^0-9.]/g,''))||0;return s+p*c.quantity;},0);return`JMD $${sum.toFixed(2)}`;};
  const submitOrder=async()=>{if(!profile||!cart.length)return;setSubmitting(true);
    try{const requestId=`ORD-${Date.now().toString(36).toUpperCase()}`;
      ...
      setCart([]);setOrderSuccess(true);showToast('Order submitted!','success');
  ...
  return(<div className="min-h-screen pt-28 pb-16 bg-slate-50 dark:bg-darkbg">
    <div className="container mx-auto px-4"><div className="max-w-6xl mx-auto">
      ...
      :<div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">{items.map(item=>(
        <article key={item.id} className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm overflow-hidden hover:shadow-md transition-shadow group">
          <div className="aspect-square overflow-hidden bg-slate-100 dark:bg-slate-700">{item.image?<img src={item.image} alt={item.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"/>:<div className="w-full h-full flex items-center justify-center"><ShoppingBag className="w-12 h-12 text-slate-300"/></div>}</div>
          <div className="p-4"><h3 className="font-black text-navy dark:text-white text-sm leading-tight mb-1">{item.name}</h3>{item.description&&<p className="text-slate-400 text-xs mb-3 line-clamp-2">{item.description}</p>}<div className="flex items-center justify-between"><span className="font-black text-ocean text-base">{item.price}</span><button onClick={()=>addToCart(item)} className="flex items-center gap-1 px-3 py-1.5 bg-navy text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-ocean transition-colors"><Plus className="w-3 h-3"/>Add</button></div></div>
        </article>
      ))}</div>}
    </div></div>
    {cartOpen&&<>
      ...
      <div className="fixed right-0 top-0 h-full w-full max-w-sm bg-white dark:bg-slate-900 z-50 flex flex-col shadow-2xl">
        ...
        {orderSuccess?<div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
          ...
          <div className="flex-1 overflow-y-auto p-5">
            ...
            :<div className="space-y-4">{cart.map(item=>(
              <div key={item.id} className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0"><p className="font-bold text-navy dark:text-white text-sm truncate">{item.name}</p><p className="text-ocean font-black text-sm">{item.price}</p></div>
                <div className="flex items-center gap-1">
                  <button onClick={()=>updateQty(item.id,-1)} className="w-6 h-6 bg-slate-100 dark:bg-slate-700 rounded-lg flex items-center justify-center"><Minus className="w-3 h-3"/></button>
                  <span className="w-6 text-center text-sm font-black text-navy dark:text-white">{item.quantity}</span>
                  <button onClick={()=>updateQty(item.id,1)} className="w-6 h-6 bg-slate-100 dark:bg-slate-700 rounded-lg flex items-center justify-center"><Plus className="w-3 h-3"/></button>
                  <button onClick={()=>setCart(p=>p.filter(c=>c.id!==item.id))} className="ml-1 p-1 text-red-300 hover:text-red-500"><Trash2 className="w-3 h-3"/></button>
                </div>
              </div>
          ...
          {cart.length>0&&<div className="p-5 border-t border-slate-100 dark:border-slate-800 space-y-3">
            <div className="grid grid-cols-2 gap-2">
              {(["walk-in","online"] as const).map(m=><button key={m} onClick={()=>setPaymentMethod(m)} className={`py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${paymentMethod===m?'bg-navy text-white':'bg-slate-50 text-slate-400'}`}>{m==="walk-in"?"Walk-in":"Online"}</button>)}
            </div>
            <button onClick={submitOrder} disabled={submitting} className="w-full py-3 bg-green-500 hover:bg-green-600 text-white font-black rounded-xl text-xs uppercase tracking-widest flex items-center justify-center gap-2 disabled:opacity-60">
