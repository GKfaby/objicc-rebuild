...
export default function ProfilePage(){
  ...
  const handleSignOut=async()=>{
    if(!window.confirm('Sign out of OBJICC?'))return;
    setSigningOut(true);
    try{await signOut(auth);navigate('/');}
    catch{showToast('Failed to sign out.','error');setSigningOut(false);}
  };
    ...
    <div className="container mx-auto px-4 max-w-3xl">
          ...
          <div className="flex items-start justify-between flex-wrap gap-4">
            <div>
              ...
              <div className="flex items-center gap-2 mt-2">
                <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-black uppercase tracking-widest ${isPending?'bg-amber-100 text-amber-700':'bg-navy/10 text-navy dark:bg-navy/30 dark:text-gold'}`}>
            ...
            <div className="flex gap-2">
              {!editing?(
                <button onClick={()=>setEditing(true)} className="flex items-center gap-2 px-4 py-2 bg-slate-100 dark:bg-slate-700 text-navy dark:text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-slate-200 transition-colors">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm p-6 space-y-5 mb-4">
        ...
        {[{label:'First Name',value:profile.firstName},{label:'Last Name',value:profile.lastName},{label:'Email',value:profile.email},...(profile.cadetName?[{label:'Cadet Name',value:profile.cadetName}]:[])].map(({label,value})=>(
          <div key={label}><label className="block text-xs font-bold text-slate-500 uppercase mb-1">{label}</label>
        ...
        <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Phone</label>
          {editing?(
        ...
        {(profile.role==='cadet'||profile.requestedRole==='cadet')&&<div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">School</label>
          {editing?(
        ...
        <div className="pt-2 flex items-center gap-2 text-xs text-slate-400">
          <Clock className="w-3 h-3"/>Member since {profile.createdAt?.toDate?.().toLocaleDateString()||'recently'}
        </div>
      </div>

      {/* Sign Out button — always visible */}
      <button onClick={handleSignOut} disabled={signingOut}
        className="w-full flex items-center justify-center gap-3 py-4 bg-white dark:bg-slate-800 text-red-500 font-black rounded-2xl shadow-sm hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors text-sm uppercase tracking-widest border border-slate-100 dark:border-slate-700 disabled:opacity-60">
