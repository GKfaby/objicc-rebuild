...
type Mode='login'|'signup'|'complete-profile'|'link-account';
...
export default function AuthForm({initialMode='login'}:{initialMode?:Mode}){
  ...
  const[pendingGoogleCred,setPendingGoogleCred]=useState<ReturnType<typeof GoogleAuthProvider.credentialFromError>>(null);
  const{schools,refreshProfile}=useUser();
  const{showToast}=useToast();
  const navigate=useNavigate();
  const location=useLocation();
  const from=(location.state as any)?.from?.pathname||'/';
  useEffect(()=>{setMode(initialMode);setError('');setForm(EF);setShowPass(false);setLinkEmail('');setLinkPassword('');setPendingGoogleCred(null);},[initialMode]);
  ...
  const handleGoogle=async()=>{setLoading(true);setError('');
    try{const r=await signInWithPopup(auth,googleProvider);const snap=await getDoc(doc(db,'users',r.user.uid));if(snap.exists())navigate(from,{replace:true});else navigate('/complete-profile',{replace:true});}
      ...
      if(err.code==='auth/account-exists-with-different-credential'){
        ...
        // Rather than silently starting a brand-new (password-less)
        // account, ask them to confirm with their EXISTING password once
        // so we can link the two — after that, Google sign-in resolves
        // straight to the same account with no password prompt.
        const email=err.customData?.email as string|undefined;
        const cred=GoogleAuthProvider.credentialFromError(err);
        if(email){
          const methods=await fetchSignInMethodsForEmail(auth,email);
          if(methods.includes('password')){
            setPendingGoogleCred(cred);setLinkEmail(email);setMode('link-account');setLoading(false);return;
          }
        }
  ...
  const handleLinkAccount=async(e:React.FormEvent)=>{e.preventDefault();setLoading(true);setError('');
    try{
      const cred=await signInWithEmailAndPassword(auth,linkEmail,linkPassword);
      if(pendingGoogleCred)await linkWithCredential(cred.user,pendingGoogleCred);
      showToast('Accounts connected — you can use Google sign-in from now on.','success');
      navigate(from,{replace:true});
    }catch(err:any){
      setError(err.code==='auth/wrong-password'||err.code==='auth/invalid-credential'?'Incorrect password.':err.message||'Something went wrong.');
      setLinkPassword('');
    }finally{setLoading(false);}};
      ...
      <div className="p-8 overflow-y-auto max-h-[78vh] custom-scrollbar">
        ...
        {mode==='link-account'?(<>
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-5 leading-relaxed">
            An account already exists for <span className="font-bold text-navy dark:text-white">{linkEmail}</span> with a password. Enter it once to connect your Google sign-in — after this, either option will get you straight in.
          </p>
          <form onSubmit={handleLinkAccount} className="space-y-4">
            <div>
              ...
              <div className="relative"><Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400"/>
                <input required autoFocus type={showPass?'text':'password'} value={linkPassword} onChange={e=>setLinkPassword(e.target.value)} className="w-full pl-10 pr-10 py-3 bg-slate-50 dark:bg-slate-800 rounded-xl text-navy dark:text-white font-bold outline-none focus:ring-2 focus:ring-navy/20 text-sm"/>
            ...
            <button type="submit" disabled={loading} className="w-full py-4 bg-navy text-white font-black rounded-xl hover:bg-ocean transition-all flex items-center justify-center gap-3 uppercase tracking-widest shadow-lg disabled:opacity-60">
              {loading?<div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"/>:<><CheckCircle2 className="w-5 h-5"/>Connect Account</>}
            </button>
          </form>
          <div className="mt-6 text-center"><button type="button" onClick={()=>navigate('/login')} className="text-xs text-slate-500 hover:text-navy dark:hover:text-white font-bold">Cancel</button></div>
        </>):(<>
