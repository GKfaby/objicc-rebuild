    ...
    <footer className="bg-navy text-white/70">
      <div className="container mx-auto px-4 py-12">
            ...
            {systemSettings.logoUrl && (
              <div className="logo-footer-wrap">
                <img
                  ...
                  onError={(e) => { (e.target as HTMLImageElement).closest('.logo-footer-wrap')?.classList.add('hidden'); }}
                />
              </div>
            )}
            ...
            <div className="flex flex-col gap-3 text-sm">
              ...
              {systemSettings.whatsappNumber && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="flex gap-2 items-center hover:text-gold transition-colors"
                >
                  <Phone className="w-4 h-4 shrink-0 text-gold" />
                  <span>+{systemSettings.whatsappNumber}</span>
                </a>
              )}
              {systemSettings.email && (
                <a href={`mailto:${systemSettings.email}`} className="flex gap-2 items-center hover:text-gold transition-colors">
        ...
        <div className="border-t border-white/10 mt-10 pt-6 text-xs text-center text-white/40">
          © {year} {systemSettings.orgName}. All rights reserved.
        </div>
      </div>
    </footer>
  );
