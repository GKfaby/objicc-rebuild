...
export default function NoticeBoard(){
  ...
  useEffect(()=>{const q=query(collection(db,'posts'),orderBy('createdAt','desc'),limit(6));return onSnapshot(q,snap=>{setPosts(snap.docs.map(d=>({id:d.id,...d.data()} as Post)));setLoading(false);});},[]);
  if(!loading&&posts.length===0)return null;
  return(<section className="py-20 bg-white dark:bg-slate-900">
    <div className="container mx-auto px-4">
      <div className="flex items-end justify-between mb-12">
        <div><div className="text-xs font-black uppercase tracking-widest text-ocean mb-2">Latest Updates</div><h2 className="text-4xl font-black text-navy dark:text-white uppercase tracking-tight">Notices & Events</h2></div>
